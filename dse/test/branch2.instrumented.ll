...
land.lhs.true:                                    ; preds = %land.lhs.true.tmp
  ...
  br i1 %cmp1, label %land.lhs.true2.tmp, label %if.end.tmp1

land.lhs.true2:                                   ; preds = %land.lhs.true2.tmp
  ...
  br i1 %cmp3, label %if.then.tmp, label %if.end.tmp2
...
if.then:                                          ; preds = %if.then.tmp
  ...
  call void @__DSE_BinOp__(i32 14, i32 19)
  store i32 %div, i32* %x, align 4
  call void @__DSE_Register__(i32 14)
  call void @__DSE_Store__(i32* %x)
  br label %if.end

if.end:                                           ; preds = %if.end.tmp2, %if.end.tmp1, %if.end.tmp, %if.then
  ret i32 0
...
land.lhs.true2.tmp:                               ; preds = %land.lhs.true
...
if.then.tmp:                                      ; preds = %land.lhs.true2
  call void @__DSE_Branch__(i32 2, i32 11, i32 1)
  br label %if.then

if.end.tmp2:                                      ; preds = %land.lhs.true2
  call void @__DSE_Branch__(i32 2, i32 11, i32 0)
  br label %if.end
}
